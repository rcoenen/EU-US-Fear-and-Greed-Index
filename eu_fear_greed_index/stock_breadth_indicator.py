...
MIN_PRICE_CHANGE = 0.0001  # 0.01% minimum change - same as US
MIN_VOLUME_RATIO = 0.5  # minimum volume ratio to average
DECLINE_WEIGHT = 7.0  # weight for declining stocks - same as US
MOMENTUM_DAYS = 10       # Days to check for downward momentum
MOMENTUM_THRESHOLD = -0.0005  # same as US
EXTREME_FEAR_THRESHOLD = 0.20  # same as US
TREND_DAYS = 5  # days to check market trend
TREND_THRESHOLD = -0.001  # same as US
...
def calculate_breadth_score(tickers=SAMPLE_TICKERS, period=LOOKBACK_PERIOD, 
    ...
    print(f"Breadth: Analyzed {valid_tickers} tickers. Advancing Vol: {total_advancing_volume:,.0f}, Declining Vol: {total_declining_volume:,.0f}")
